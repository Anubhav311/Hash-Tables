...
def array_pop(array, index):
    ...
    # Have the helper check for above and below bound
    if index >= array.count or index < 0:
        print(f"Error, element {index} is out of bound")
        return None

    return_value = array.elements[index]

    #TODO write an array shift function for here and array remove
